            ...
            if (e.Control && e.KeyCode == Keys.V)
            {
                try
                {
                    if (Clipboard.GetDataObject().GetDataPresent(DataFormats.Text))
                	    this.NumericValue=Convert.ToDecimal(Clipboard.GetDataObject().GetData(DataFormats.Text).ToString());
                }
                catch { this.NumericValue = 0; }
                return;
            }
            else if (e.Control && e.KeyCode == Keys.C)
            {
                Clipboard.SetDataObject(this.NumericValue.ToString());
                return;
            }
			switch(e.KeyCode) 
			{
			...
			if ( e.KeyChar.Equals('-') ) 
			{
				if ( this.AllowNegative ) 
				{
					if ( this.Text.IndexOf('-') < 0 ) 
					{
                        ...
                        if (!(Convert.ToDecimal(this.Text) == 0)) //input 0
                        {
                            this.Text = "-" + this.Text;
